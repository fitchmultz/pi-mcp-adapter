...
With `configPath` and no `config`, the adapter keeps normal file merge behavior, and that path takes precedence over argv and `--mcp-config`. The default export keeps the normal file-based behavior. OAuth credentials are stored per configured server name as AES-256-GCM encrypted files under the OAuth directory (default `~/.pi/agent/mcp-oauth`), with the single shared encryption key held in the operating system credential store — so macOS shows at most one keychain prompt per install. URL binding prevents credentials from being accepted for a different server URL. `settings.oauthDir` and `MCP_OAUTH_DIR` select the credential directory (and remain the import source for older plaintext `tokens.json` files), so explicitly configured directories also scope credentials per directory. CSRF state and PKCE verifiers are flow-local, so concurrent authorization flows do not share transient secrets.
...
If Pi is running on a remote server and cannot open a local browser, start OAuth through the proxy tool. Persistent OAuth still requires an available OS credential store (it holds the single encryption key for the encrypted credential files); on headless Linux that usually means an unlocked Secret Service/libsecret keyring. The adapter fails closed instead of falling back to plaintext credentials when the secure store is unavailable.

On Linux, if credential access fails because Pi inherited a revoked session keyring, the adapter uses a best-effort recovery path through `keyctl session - node <packaged helper>` so explicit re-authentication can write fresh credentials without killing a long-lived tmux server. This path only touches the single encryption key (plus one-time reads/removals of legacy entries during migration) and requires `keyctl` and `node` on `PATH`; missing, locked, or otherwise unavailable credential stores still fail closed.
...
| `oauthDir` | OAuth credential directory for this MCP config: encrypted `sha256-<server-hash>.enc` files live here (the encryption key stays in the OS credential store), and legacy plaintext `tokens.json` files are imported from here. Relative paths resolve from the active project cwd. `MCP_OAUTH_DIR` still wins when set. Never commit a project-local OAuth directory. |
