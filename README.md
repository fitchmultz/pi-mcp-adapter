...
`createMcpAdapter({ beforeExecute: async (toolCallId, ctx) => { ... } })` lets a host finish its workspace/native-session checkpoint before the next MCP action. It is awaited before registered direct-tool and `mcp` execution, and before every resolved `mcp_script` inner call. For scripts, it runs after target resolution, connection and approval but before the call deadline and `onToolCall` capture; the script's own timeout still applies. A second awaited script call reaches this boundary after the first call's output files have finished writing. The callback receives the native Pi call ID and `ExtensionContext`; pass `ctx.signal` to checkpoint I/O and throw if the checkpoint cannot complete.

Supplying `beforeExecute` marks the adapter's tools `executionMode: "sequential"`, so Pi finishes earlier sibling tools before entering this boundary. Omitting it keeps normal scheduling. It does not serialize `Promise.all` inside a script, join detached writers, or implement checkpoint storage. The host still owns coherent capture and a final completion checkpoint, including final script output files. No request or script timeout defaults change.
