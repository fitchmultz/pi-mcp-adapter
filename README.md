...
`createMcpAdapter({ beforeExecute: async (toolCallId, ctx) => { ... } })` lets a host finish its workspace/native-session checkpoint before the next MCP action. It is awaited before registered direct-tool and `mcp` execution, and before every resolved `mcp_script` inner call. For scripts, it runs after target resolution, connection and approval but before the call deadline and `onToolCall` capture; any configured overall script deadline still applies. A second awaited script call reaches this boundary after the first call's output files have finished writing. The callback receives the native Pi call ID and `ExtensionContext`; pass `ctx.signal` to checkpoint I/O and throw if the checkpoint cannot complete.
